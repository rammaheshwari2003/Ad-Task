  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      imageFiles.forEach((file) => {
        productData.append("images", file); // Field name: "images"
      });

      // Append PDF (single file)
      if (imageFilesPdf.length > 0) {
        productData.append("PDFbrochure", imageFilesPdf[0]); // Field name: "PDFbrochure"
      }
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div>
          ...
          <label className="flex items-center justify-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-primary-500">
            ...
            <input
              ...
              className="hidden"
              disabled={imageFilesPdf.length >= 1}
            />
          </label>
        </div>
        ...
        <div>
          <button
            type="submit"
            disabled={loading}
            className={`w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 ${loading ? "opacity-70 cursor-not-allowed" : ""
              }`}
          >
